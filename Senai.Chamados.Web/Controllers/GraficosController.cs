        ...
        public ActionResult Index()
        {
            try
            {
                ...
                using (ChamadoRepositorio _repoChamado = new ChamadoRepositorio())
                {
                    ...
                    #region Grafico Staus


                    var grupoStaus = vmListaChamados.ListaChamados
                        .GroupBy(x => x.Status)
                        .Select(n => new
                        {
                    ...
                    vmGrafico.GraficoStatus.Labels = grupoStaus.Select(x => x.Status).ToArray();
                    vmGrafico.GraficoStatus.Data = grupoStaus.Select(x => x.Quantidade).ToArray();
